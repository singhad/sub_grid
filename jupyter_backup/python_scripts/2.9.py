...
if __name__=='__main__':
    ...
    c_s_arr = np.sqrt(K_b * region.g["temp"] / m_p)
    ...
    for m in range(0, len(mach_no_arr)):
        ...
        Z = Z_arr[m]
        c_s = c_s_arr[m]
        X_H2_bar[m], X_CO_bar[m] = inside_loop(mach_no, n_H_mean, Z, G_o, c_s, m_p)

    np.save('outputs/2.9/X_H2_bar_2.9.npy', X_H2_bar)
    np.save('outputs/2.9/X_CO_bar_2.9.npy', X_CO_bar)
    np.save('outputs/2.9/mach_no_arr_2.9.npy', mach_no_arr)
    np.save('outputs/2.9/n_H_mean_arr_2.9.npy', n_H_mean_arr)
    np.save('outputs/2.9/Z_arr_2.9.npy', Z_arr)