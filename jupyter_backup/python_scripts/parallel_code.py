...
if __name__ == '__main__':
    ...
    for j, cell in enumerate(rel_cells):
        X_H2_bar, X_CO_bar = X_list[j]
        X_H2_bar_cells[cell] = X_H2_bar
        X_CO_bar_cells[cell] = X_CO_bar

    np.save('outputs/parallel_code/X_H2_bar_' + run + '_' + out + '.npy', X_H2_bar_cells)
    np.save('outputs/parallel_code/X_CO_bar_' + run + '_' + out + '.npy', X_CO_bar_cells)
    np.save('outputs/parallel_code/mach_no_arr_' + run + '_' + out + '.npy', mach_no_sim)
    np.save('outputs/parallel_code/n_H_mean_arr_' + run + '_' + out + '.npy', n_H_mean_sim)
    np.save('outputs/parallel_code/Z_arr_' + run + '_' + out + '.npy', Z_arr)
    np.save('outputs/parallel_code/T_' + run + '_' + out + '.npy', temperature)
