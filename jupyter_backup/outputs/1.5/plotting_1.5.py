...
get_ipython().run_cell_magic(u'time', u'', u"#multiplied X_H2_bar with 2 during the 1.1 iteration\nX_CO_bar = np.load('outputs/1.5/X_CO_bar_1.5.npy')\nmin_C = np.min(X_CO_bar)\nmax_C = np.max(X_CO_bar)")
...
# ax.plot_trisurf(X_H2_bar, np.log10(n_H_mean_sim), mach_no_sim, 
#                    norm=Z_arr, vmin=min_Z, vmax=max_Z, cmap='viridis')
# plt.colorbar(label="$Z$")
# plt.xlabel('log(n_H_mean)')
# plt.ylabel('X_H2_bar')
# plt.zlabel('Mach no.')
