...
def tau_LVG(n_CO, nu, lambda_jeans, x_1, x_0, B_10, B_01, c_cgs, h_ev, sigma_turb, cell_width):
    # units: eV*s * Hz * cm * 1/cm3 * cm2/(eV*s) * 1/(Hz) = none
    delta_v = (sigma_turb)*((lambda_jeans/cell_width)**0.5)
    delta_nu = delta_v*nu/c_cgs #Hz
    return np.asarray(h_ev*nu*lambda_jeans*n_CO*((x_0*B_01)-(x_1*B_10))/(4*np.pi*delta_nu))
...
def inside_loop(mach_no, n_H_mean, metal, G_o, T_mean, m_p, K_b, 
    ...
    tau_nu = tau_LVG(n_CO, nu, lambda_jeans, x_1, x_0, 
                     B_10, B_01, c_cgs, h_ev, sigma_turb, cell_width)
    ...
    convert_units = ((c_cgs/nu)**3)*1e-5*((3.24078e-19)**2)/(2*K_b_erg)
    #conversion factor: erg/cm3/s to K km s-1 pc2
    l_CO_SI = l_CO * convert_units # K km s-1 pc2
#     alpha_CO = l_CO/(m_p*n_H)
...
#     print (x_0)
    ...
    def map_to_loop(cell):
        ...
        X_H2_bar, X_CO_bar, l_CO_bar, l_CO_SI_bar, m_H2_bar = \
    ...
    np.save('outputs/debug_1.4/X_H2_bar_' + run + '_' + out + '.npy', \
            X_H2_bar_cells)
    np.save('outputs/debug_1.4/X_CO_bar_' + run + '_' + out + '.npy', \
            X_CO_bar_cells)
    np.save('outputs/debug_1.4/mach_no_arr_' + run + '_' + out + '.npy', \
            mach_no_sim)
    np.save('outputs/debug_1.4/n_H_mean_arr_' + run + '_' + out + '.npy', \
            n_H_mean_sim)
    np.save('outputs/debug_1.4/metal_arr_' + run + '_' + out + '.npy', metal_arr)
    np.save('outputs/debug_1.4/T_' + run + '_' + out + '.npy', temperature)
    np.save('outputs/debug_1.4/l_CO_bar_' + run + '_' + out + '.npy', \
            l_CO_bar_cells)
    np.save('outputs/debug_1.4/l_CO_SI_bar_' + run + '_' + out + '.npy', \
            l_CO_SI_bar_cells)
    np.save('outputs/debug_1.4/m_H2_bar_' + run + '_' + out + '.npy', \
            m_H2_bar_cells) 
    np.save('outputs/debug_1.4/mass_' + run + '_' + out + '.npy', mass)
...
#     delta_v_2 = c_s_CO*nu/c_cgs
    
#     delta_nu_1 = delta_v_1*nu/c_cgs #Hz
#     delta_nu_2 = delta_v_2*nu/c_cgs #Hz
...
#     np.save('outputs/debug_1.4/metal_arr_' + run + '_' + out + '.npy', metal_arr)
#     np.save('outputs/debug_1.4/T_' + run + '_' + out + '.npy', temperature)
#     np.save('outputs/debug_1.4/l_CO_bar_1' + run + '_' + out + '.npy', \
#             l_CO_bar_1_cells)
#     np.save('outputs/debug_1.4/l_CO_SI_bar_1' + run + '_' + out + '.npy', \
#             l_CO_SI_bar_1_cells)
#     np.save('outputs/debug_1.4/l_CO_bar_2' + run + '_' + out + '.npy', \
#             l_CO_bar_2_cells)
#     np.save('outputs/debug_1.4/l_CO_SI_bar_2' + run + '_' + out + '.npy', \
#             l_CO_SI_bar_2_cells)
#     np.save('outputs/debug_1.4/m_H2_bar_' + run + '_' + out + '.npy', \
#             m_H2_bar_cells) 
#     np.save('outputs/debug_1.4/mass_' + run + '_' + out + '.npy', mass)
