...
def inside_loop(mach_no, n_H_mean, metal, G_o, T_mean, T, G, m_p, K_b, K_b_ev, K_b_erg, c_cgs, h_ev, T_bg, eV, s, n_H, pdf,
    ...
    l_CO = j_10 * beta_nu * eV_to_ergs/m_p  # ergs/cm3/s/g
    M_H2 = n_H * m_p * 4 * np.pi * (cell_width**3)/3  #g
    alpha_CO = ((c_cgs/nu)**3)*1e-5*((3.24078e-19)**2)*M_H2/ (2*K_b_erg) #conversion factor: eV/cm3/s/g to K km s-1 pc2 Msol-1
    l_CO_SI = l_CO * alpha_CO # K km s-1 pc2
    
    M_H2_bar = np.sum(M_H2*pdf*ds)
...
if __name__=='__main__':
    ...
    M_H2_bar = np.zeros(len(n_H_mean_arr))
    ...
    for m in range(0, len(mach_no_arr)):
        ...
        X_H2_bar[m], X_CO_bar[m], l_CO_bar[m], l_CO_SI_bar[m], M_H2_bar[m] = inside_loop(mach_no, n_H_mean, 
                                                            metal, G_o, T_mean, T, G, m_p, 
    ...
    np.save('outputs/pdf_RT_1.1/X_H2_bar.npy', X_H2_bar)
    np.save('outputs/pdf_RT_1.1/X_CO_bar.npy', X_CO_bar)
    np.save('outputs/pdf_RT_1.1/mach_no_arr.npy', mach_no_arr)
    np.save('outputs/pdf_RT_1.1/n_H_mean_arr.npy', n_H_mean_arr)
    np.save('outputs/pdf_RT_1.1/metal_arr.npy', metal_arr)
    np.save('outputs/pdf_RT_1.1/T.npy', temperature)
    np.save('outputs/pdf_RT_1.1/l_CO_bar.npy', l_CO_bar)
    np.save('outputs/pdf_RT_1.1/l_CO_SI_bar.npy', l_CO_SI_bar)
    np.save('outputs/pdf_RT_1.1/M_H2_bar.npy', M_H2_bar)