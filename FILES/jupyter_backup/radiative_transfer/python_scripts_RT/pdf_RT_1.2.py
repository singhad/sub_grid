...
def inside_loop(mach_no, n_H_mean, metal, G_o, T_mean, T, G, m_p, K_b, K_b_ev, K_b_erg, c_cgs, h_ev, T_bg, eV, s, n_H, pdf,
    ...
    m_H = m_p * (cell_width**3)*X_H2_ss  #g
    l_CO = j_10 * beta_nu * eV_to_ergs * m_H/m_p  # ergs/s
    
    alpha_CO = ((c_cgs/nu)**3)*1e-5*((3.24078e-19)**2)/(2*K_b_erg) #conversion factor: erg/s to K km s-1 pc2 Msol-1
...
if __name__=='__main__':
    ...
    for m in range(0, len(mach_no_arr)):
        ...
        X_H2_bar[m], X_CO_bar[m], l_CO_bar[m], l_CO_SI_bar[m] = inside_loop(mach_no, n_H_mean, 
    ...
    np.save('outputs/pdf_RT_1.2/X_H2_bar.npy', X_H2_bar)
    np.save('outputs/pdf_RT_1.2/X_CO_bar.npy', X_CO_bar)
    np.save('outputs/pdf_RT_1.2/mach_no_arr.npy', mach_no_arr)
    np.save('outputs/pdf_RT_1.2/n_H_mean_arr.npy', n_H_mean_arr)
    np.save('outputs/pdf_RT_1.2/metal_arr.npy', metal_arr)
    np.save('outputs/pdf_RT_1.2/T.npy', temperature)
    np.save('outputs/pdf_RT_1.2/l_CO_bar.npy', l_CO_bar)
    np.save('outputs/pdf_RT_1.2/l_CO_SI_bar.npy', l_CO_SI_bar)