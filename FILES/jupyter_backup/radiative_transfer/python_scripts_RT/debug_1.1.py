...
def inside_loop(mach_no, n_H_mean, metal, G_o, T_mean, m_p, K_b, 
                ...
                B_10, B_01, nu, x_1, x_0, eV_to_ergs, cell_width, M_sun):
    ...
    l_CO_SI = l_CO * alpha_CO # K km s-1 pc2 M_sol-1 cm-3
...
if __name__ == '__main__':
    ...
    u = 1 #upper level
    l = 0 #lower level
    nu = 115.2712018e9 #freq[u][l]
    x_1 = 0.43479505590767203 #ni[u]
    x_0 = 0.2520174421256517 #ni[l]
    A_10 = 7.203e-08 #A[u][l]
    B_10 = 5.10995088335e-06 #B[u][l]
    B_01 = 1.532985265e-05  #B[l][u]    
...
#     print (x_0)
    ...
    def map_to_loop(cell):
        ...
        inside_loop(mach_no, n_H_mean, metal, G_o, T_mean, m_p, K_b, 
            ...
            nu, x_1, x_0, eV_to_ergs, cell_width, M_sun)
        return X_H2_bar, X_CO_bar, l_CO_bar, l_CO_SI_bar, m_H2_bar
    ...
    np.save('outputs/debug_1.1/X_H2_bar_' + run + '_' + out + '.npy', \
            X_H2_bar_cells)
    np.save('outputs/debug_1.1/X_CO_bar_' + run + '_' + out + '.npy', \
            X_CO_bar_cells)
    np.save('outputs/debug_1.1/mach_no_arr_' + run + '_' + out + '.npy', \
            mach_no_sim)
    np.save('outputs/debug_1.1/n_H_mean_arr_' + run + '_' + out + '.npy', \
            n_H_mean_sim)
    np.save('outputs/debug_1.1/metal_arr_' + run + '_' + out + '.npy', metal_arr)
    np.save('outputs/debug_1.1/T_' + run + '_' + out + '.npy', temperature)
    np.save('outputs/debug_1.1/l_CO_bar_' + run + '_' + out + '.npy', \
            l_CO_bar_cells)
    np.save('outputs/debug_1.1/l_CO_SI_bar_' + run + '_' + out + '.npy', \
            l_CO_SI_bar_cells)
    np.save('outputs/debug_1.1/m_H2_bar_' + run + '_' + out + '.npy', \
            m_H2_bar_cells)
    np.save('outputs/debug_1.1/mass_' + run + '_' + out + '.npy', mass)