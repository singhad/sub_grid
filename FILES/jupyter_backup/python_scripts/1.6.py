...
def calc_n_LW_ss(n_H, n_H2, G_o, lambda_jeans, Z):
    kappa = 1000 * m_p * (Z/0.02)
    rad_field_outside = G_o #in solar units
    exp_tau = np.exp(-kappa * n_H * lambda_jeans)
...
def self_shielding_iterations(n_H, G_o, lambda_jeans, Z):
    ...
    while i<ctr:
        #for j in range(100):
        n_LW_ss = calc_n_LW_ss(n_H, n_H2_ss, G_o, lambda_jeans, Z)
...
if __name__=='__main__':
    ...
    np.save('outputs/1.6/X_H2_bar_1.6.npy', X_H2_bar)
    np.save('outputs/1.6/X_CO_bar_1.6.npy', X_CO_bar)
    np.save('outputs/1.6/mach_no_arr_1.6.npy', mach_no_arr)
    np.save('outputs/1.6/n_H_mean_arr_1.6.npy', n_H_mean_arr)
