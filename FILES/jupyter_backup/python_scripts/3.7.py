# Here, Z is taken in mass fraction units:
#     Z_sim is in mass fraction units
#     Z_arr = Z_sim
#     kappa = 1000*m_p*(Z_arr/0.02)  --> in n_LW & n_LW_ss
#     denominator = CC * (Z_arr/0.02) * n_H  --> in X_H2
#     n_CO = 1e-4*n_H*X_CO*(Z_arr/0.02)
...
def self_shielding_iterations(n_H, G_o, lambda_jeans, Z, m_p, n_H_mean, T):
    n_LW = np.zeros(1000)
    X_H2 = np.zeros(1000)
    n_H2 = np.zeros(1000)
...
def calc_X_CO(n_H, n_H2, n_LW, n_H_mean, T):
...
def inside_loop(mach_no, n_H_mean, Z, G_o, c_s, m_p, T, s, n_H, pdf, lambda_jeans, X_CO, n_CO, integral1, integral2, sigma_s,
                s_bar, smin, smax, ds):
    s = 0 * s
    n_H = 0 * n_H
    pdf = 0 * pdf
    ...
    if (T>=1e4) | (n_H_mean <= 1e-2) :
        ...
        n_LW, n_H2, n_LW_ss, X_H2_ss, n_H2_ss = self_shielding_iterations(n_H, G_o, lambda_jeans, Z, m_p, n_H_mean, T)
        integral1 = calc_integral1(s, pdf, X_H2_ss, ds)
        X_H2_bar = 2 * integral1

        X_CO = calc_X_CO(n_H, n_H2, n_LW, n_H_mean, T)
...
if __name__=='__main__':
    ...
    for m in range(0, len(mach_no_arr)):
        ...
        c_s = c_s_arr[m]
        T = temperature[m]
        X_H2_bar[m], X_CO_bar[m] = inside_loop(mach_no, n_H_mean, Z, G_o, c_s, m_p, T, s, n_H, pdf, lambda_jeans, X_CO, n_CO,
                                               integral1, integral2, sigma_s, s_bar, smin, smax, ds)

    np.save('outputs/3.7/X_H2_bar_3.7.npy', X_H2_bar)
    np.save('outputs/3.7/X_CO_bar_3.7.npy', X_CO_bar)
    np.save('outputs/3.7/mach_no_arr_3.7.npy', mach_no_arr)
    np.save('outputs/3.7/n_H_mean_arr_3.7.npy', n_H_mean_arr)
    np.save('outputs/3.7/Z_arr_3.7.npy', Z_arr)