...
def inside_loop(mach_no, n_H_mean, Z, G_o, T_mean, T, m_p, K_b, s, n_H, pdf, lambda_jeans, X_CO, n_CO, integral1, integral2,
    ...
    if (T>=1e4) | (n_H_mean <= 1e-2) :
        ...
        n_LW, n_H2, n_LW_ss, X_H2_ss, n_H2_ss = self_shielding_iterations(n_H, G_o, lambda_jeans, Z, m_p)
...
if __name__=='__main__':
    ...
    for m in range(0, len(Z_arr)):
        mach_no = pynbody.array.SimArray(10., "1")
        n_H_mean = n_H_mean_arr[m]
        Z = Z_arr[m]
        T = pynbody.array.SimArray(10., "K")
        X_H2_bar[m], X_CO_bar[m] = inside_loop(mach_no, n_H_mean, Z, G_o, T_mean, T, m_p, K_b, s, n_H, pdf, lambda_jeans, X_CO,
                                               n_CO, integral1, integral2, sigma_s, s_bar, smin, smax, ds)

    np.save('outputs/3.9/X_H2_bar_3.9.npy', X_H2_bar)
    np.save('outputs/3.9/X_CO_bar_3.9.npy', X_CO_bar)
    np.save('outputs/3.9/mach_no_arr_3.9.npy', mach_no_arr)
    np.save('outputs/3.9/n_H_mean_arr_3.9.npy', n_H_mean_arr)
    np.save('outputs/3.9/Z_arr_3.9.npy', Z_arr)
    np.save('outputs/3.9/T_3.9.npy', temperature)