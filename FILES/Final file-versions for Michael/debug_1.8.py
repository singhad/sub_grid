...
# USING non-LTE+PDF EVOLUTION TO CALCULATE X_H2_BAR, X_CO_BAR, l_CO_BAR
...
def calc_integral(s, pdf_prime, X, ds):
    return np.sum(np.exp(s) * pdf_prime * X * ds)
...
def inside_loop(mach_no, n_H_mean, metal, G_o, T_mean, m_p, K_b, 
                ...
                B_10, B_01, nu, x_1, x_0, eV_to_ergs, cell_width, 
                M_sun, sigma_turb, j_10_1, j_10_2):
    ...
    pdf = make_pdf(s, s_bar, sigma_s) #log-normal PDF
    
    gamma = (1+(2*eps*np.exp(s))-np.sqrt(1+(4*eps*np.exp(s))))/(2*(eps**2)*np.exp(s))
    s_prime = np.log(gamma)
    q = ((1-(eps*np.exp(s_prime)))**3)/(1+(eps*np.exp(s_prime)))
    pdf_prime = make_pdf(s_prime, s_bar, sigma_s) * q
    norm = np.sum(np.exp(s)*pdf_prime*ds)
    pdf_prime = pdf_prime/norm        #power-law PDF
    ...
    X_H2_bar = calc_integral(s, pdf_prime, X_H2_ss, ds)
    X_CO = calc_X_CO(n_H, n_H2, n_LW)
    n_CO = calc_n_CO(n_H, X_CO, metal)
    X_CO_bar = calc_integral(s, pdf_prime, X_CO, ds)
    ...
    T_exc_1 = (10.*(1.0-beta_nu_1))+(3.*beta_nu_1)
    T_exc_2 = (10.*(1.0-beta_nu_2))+(3.*beta_nu_2)
    
    for ctr1 in range(0, len(T_exc_1)):
        ni_1, Z_1 = calc_lvlpops_partion(T_exc_1[ctr1], num_lvls, g, E, K_b_ev)
        x_1_1 = ni_1[1]
        #print x_1_3, T_exc[ctr1], Z_3,E[]
        j_10_1[ctr1] = calc_integrated_emissivity(n_CO[ctr1], nu, x_1_1, A_10, h_ev)  # eV s-1 cm-3
        
    for ctr2 in range(0, len(T_exc_2)):
        ni_2, Z_2 = calc_lvlpops_partion(T_exc_2[ctr2], num_lvls, g, E, K_b_ev)
        x_1_2 = ni_2[1]
        #print x_1_3, T_exc[ctr2], Z_3,E[]
        j_10_2[ctr2] = calc_integrated_emissivity(n_CO[ctr2], nu, x_1_2, A_10, h_ev)  # eV s-1 cm-3
            
    #j_10 = calc_integrated_emissivity(n_CO, nu, x_1, A_10, h_ev)  # eV s-1 cm-3
    
    m_H2 = m_p * (cell_width**3) * X_H2_ss  #g cm3
    l_CO_1 = j_10_1 * beta_nu_1 * eV_to_ergs  #erg s-1 cm-3 #luminosity density
    l_CO_2 = j_10_2 * beta_nu_2 * eV_to_ergs  #erg s-1 cm-3 #luminosity density
    ...
    m_H2_bar = np.sum(m_H2*n_H*pdf_prime*ds)
    l_CO_bar_1 = np.sum(l_CO_1*pdf_prime*ds)         #mean luminosity density of each simulation cell
    l_CO_SI_bar_1 = np.sum(l_CO_SI_1*pdf_prime*ds)   #mean luminosity density of each simulation cell
    l_CO_bar_2 = np.sum(l_CO_2*pdf_prime*ds)         #mean luminosity density of each simulation cell
    l_CO_SI_bar_2 = np.sum(l_CO_SI_2*pdf_prime*ds)   #mean luminosity density of each simulation cell
...
#     print (x_0)
    ...
    def map_to_loop(cell):
        ...
        inside_loop(mach_no, n_H_mean, metal, G_o, T_mean, m_p, K_b, 
            ...
            nu, x_1, x_0, eV_to_ergs, cell_width, M_sun, sigma_turb, j_10_1, j_10_2)
        return X_H2_bar, X_CO_bar, m_H2_bar, l_CO_bar_1, l_CO_SI_bar_1, l_CO_bar_2, l_CO_SI_bar_2
    ...
    np.save('outputs/debug_1.8/X_H2_bar_' + run + '_' + out + '.npy', \
            X_H2_bar_cells)
    np.save('outputs/debug_1.8/X_CO_bar_' + run + '_' + out + '.npy', \
            X_CO_bar_cells)
    np.save('outputs/debug_1.8/mach_no_arr_' + run + '_' + out + '.npy', \
            mach_no_sim)
    np.save('outputs/debug_1.8/n_H_mean_arr_' + run + '_' + out + '.npy', \
            n_H_mean_sim)
    np.save('outputs/debug_1.8/metal_arr_' + run + '_' + out + '.npy', metal_arr)
    np.save('outputs/debug_1.8/T_' + run + '_' + out + '.npy', temperature)
    np.save('outputs/debug_1.8/l_CO_bar_1' + run + '_' + out + '.npy', \
            l_CO_bar_1_cells)
    np.save('outputs/debug_1.8/l_CO_SI_bar_1' + run + '_' + out + '.npy', \
            l_CO_SI_bar_1_cells)
    np.save('outputs/debug_1.8/l_CO_bar_2' + run + '_' + out + '.npy', \
            l_CO_bar_2_cells)
    np.save('outputs/debug_1.8/l_CO_SI_bar_2' + run + '_' + out + '.npy', \
            l_CO_SI_bar_2_cells)
    np.save('outputs/debug_1.8/m_H2_bar_' + run + '_' + out + '.npy', \
            m_H2_bar_cells) 
    np.save('outputs/debug_1.8/mass_' + run + '_' + out + '.npy', mass)
